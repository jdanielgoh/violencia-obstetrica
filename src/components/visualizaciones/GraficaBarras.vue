...
<script>
	...
	export default {
		...
		props: {
			...
			margen: {
				type: Object,
				default: function(){return {arriba:20,abajo:50,izquierda:40,derecha:20}}
			},
			alto_vis: {
				type: Number,
		...
		watch:{
			...
			datos(new_val,old_val){
                ...
                this.barras_apiladas =this.grupo_contenedor
                    .selectAll("g.grupos-barras").remove()
		...
		methods:{
			...
			configurandoDimensionesParaBarras(){
                ...
                this.eje_x.selectAll("text").style("text-anchor","end")
					.attr("transform","rotate(-60)translate(-6,-12)")
                
                if(this.nombre_barra=="fecha"){
                    this.eje_x.selectAll("text").filter(d=>parseInt(d)%5 != 0).style("opacity","0")
                }
